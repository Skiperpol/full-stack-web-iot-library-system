...
async function seed() {
  ...
  const savedCards = await cardRepo.save([
  ...
  const book3 = bookRepo.create({ cardId: cardIdByUid['CARDUID-BOOK-3'], title: 'Trzecia Książka', author: 'Autor Trzeci' });
  const book4 = bookRepo.create({ cardId: cardIdByUid['CARDUID-BOOK-4'], title: 'Czwarta Książka', author: 'Autor Czwarty' });
