...
\begin{enumerate}[label=\textbf{W1.1.3.\arabic*}]
    ...
    \item System musi umożliwiać zwrot książki przy pomocy zeskanowania karty klienta, a następnie karty książki.
...
\begin{lstlisting}[language=Python, caption={Odczyt karty RFID - rfid\_reader.py},
    showspaces=false, 
...
Funkcja \texttt{wait\_for\_card()} w sposób ciągły monitoruje obecność karty RFID przy użyciu pętli blokującej. W momencie wykrycia tagu, czytnik pobiera jego unikalny identyfikator (UID), a następnie konwertuje go do formatu listy bajtów, ciągu szesnastkowego oraz liczby całkowitej.
...
\paragraph{Opcja A: Uruchomienie w kontenerze Docker}
Ta metoda pozwala na szybkie odizolowanie brokera od reszty systemu.
...
\paragraph{Opcja B: Uruchomienie w systemie Raspberry Pi OS (według instrukcji)}
W zestawach laboratoryjnych broker Mosquitto jest już zainstalowany, lecz domyślnie nie uruchamia się przy starcie systemu.

\begin{enumerate}
    \item \textbf{Instalacja (jeśli wymagana):} Jeśli system nie posiada brokera, należy go zainstalować komendą:
    \begin{verbatim}
sudo apt update
sudo apt install mosquitto mosquitto-clients
    \end{verbatim}
    
    \item \textbf{Uruchomienie usługi:} Start brokera przeprowadzamy poleceniem:
    \begin{verbatim}
sudo systemctl start mosquitto.service
    \end{verbatim}
    
    \item \textbf{Weryfikacja:} Sprawdzenie statusu działania usługi:
    \begin{verbatim}
sudo systemctl status mosquitto.service
    \end{verbatim}
    
    \item \textbf{Dostęp zdalny:} Aby umożliwić dostęp z innych urządzeń w sieci izolowanej, należy do pliku \texttt{/etc/mosquitto/mosquitto.conf} dopisać:
    \begin{verbatim}
allow_anonymous true
listener 1883 0.0.0.0
    \end{verbatim}
    Następnie zrestartować usługę: \texttt{sudo systemctl restart mosquitto.service}.
\end{enumerate}
...
\begin{enumerate}
    ...
    \begin{itemize}
        ...
        \item Zeskanuj kartę książki (system automatycznie utworzy wypożyczenie lub zwróci wypożyczoną książkę).
...
\begin{itemize}
    ...
    \item Dodanie nowych funkcjonalnośći do Frontendu i Backendu.
\end{itemize}
